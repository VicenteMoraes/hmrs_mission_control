...
RUN pip3 install lagom
COPY requirements.txt /requirements.txt
RUN pip3 install -r /requirements.txt

# Making ros package
RUN mkdir -p action_ws/src
RUN bash -c "source /opt/ros/$ROS_DISTRO/setup.bash && cd action_ws/src && ros2 pkg create deeco_actions"
...
RUN bash -c "source /opt/ros/$ROS_DISTRO/setup.bash && cd action_ws && colcon build"
